        ...
        <div class="col-xl-6 col-md-6 ">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Program Studi
                    </h3>
                </div>
                <div class="card-body">
                    <div class="col-md-12 col-xl-12">
                        <form action="{{ route('prodi.update', $prodi->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="name" value="{{ $prodi->name }}">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Kode</label>
                                <input type="text" class="form-control" name="kode" value="{{ $prodi->kode }}">
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="fakultas_id" name="fakultas_id">
                                    ...
                                    @foreach ($fakultas as $fakulty)
                                        <option value="{{ $fakulty->id }}"
                                            @if ($prodi->fakultas_id == $fakulty->id) selected @endif>
                                            {{ $fakulty->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-select" id="jenjang_id" name="jenjang_id">
                                    ...
                                    @foreach ($jenjangs as $jenjang)
                                        <option value="{{ $jenjang->id }}"
                                            @if ($prodi->jenjang_id == $jenjang->id) selected @endif>
                                            {{ $jenjang->name }}
                                        </option>
                                    @endforeach
                                </select>
